...
for i in obsLons.index:
    if lonsize[0]<obsLons[i]<lonsize[1]:
        if abs(obsDepth[i][-1]-depthBottom[i])<10:     #diff<10m,we can use the data
...
for i in [0]:        #two seasons together plot.
    ...
    CS = ax.contourf(lon_i, lat_i, modtemp_i, np.arange(0,m_mod,0.1), cmap=plt.cm.rainbow,
                  vmax=m_mod, vmin=0)
    ...
    draw_basemap(fig, ax1, lonsize, latsize,fontsize=8)
    CS = ax1.contourf(lon_i, lat_i, obstemp_i, np.arange(0,m_obs,0.1), cmap=plt.cm.rainbow,
                  vmax=m_obs, vmin=0)
    CS1=ax1.contour(lon_is, lat_is,depth_i,1,colors = 'r',linestyles=':',linewidths=2)
    cbar=plt.colorbar(CS,ticks=np.arange(0,int(m_obs),3))
    cbar.ax.tick_params(labelsize=10)
    #cbar.ax.set_ylabel('Temperature($^\circ$C)', fontsize=20)
    #plt.scatter(np.array(obsLon),np.array(obsLat), marker='o', c='b', s=5, zorder=10)
    ...
    draw_basemap(fig, ax2, lonsize, latsize,fontsize=8)
    CS = ax2.contourf(lon_i, lat_i, modtemp_i, np.arange(0,m_mod,0.1), cmap=plt.cm.rainbow,
                  vmax=m_mod, vmin=0)#vmax=modtemp_i.max(), vmin=modtemp_i.min()
    CS1=ax2.contour(lon_is, lat_is,depth_i,1,colors = 'r',linestyles=':',linewidths=2)
    cbar=plt.colorbar(CS,ticks=np.arange(0,int(m_mod),3))#,ticks=np.arange(int(modtemp_i.min())-1,int(modtemp_i.max())+1,2)
    cbar.ax.tick_params(labelsize=10)
    #cbar.ax.set_ylabel('Temperature($^\circ$C)', fontsize=20)
    #plt.scatter(np.array(obsLon),np.array(obsLat), marker='o', c='b', s=5, zorder=10)
    ...
    draw_basemap(fig, ax3, lonsize, latsize,fontsize=8)
    CS = ax3.contourf(lon_i, lat_i, obstemp_i, np.arange(0,m_obs,0.1), cmap=plt.cm.rainbow,
                  vmax=m_obs, vmin=0)
    ...
    fig.text(0.94, 0.5, 'Temperature($^\circ$C)', ha='center', va='center', rotation='vertical',fontsize=18)  
