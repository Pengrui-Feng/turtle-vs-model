        ...
        if early_id_2011[i]==after_id_2011[j]:
            if max(obsDepth[i])>30:
                if max(obsDepth[j])>30:
                    ...
                    plt.text(7,4,'id:'+str(early_id_2011[i])+'',fontsize=12)
                    ax.set_xlim([5, 25])
                    ax.set_ylim([50, 0])
                    ...
                    ax.plot(obstemp[i],obsDepth[i],'bo--',markersize=4, label=str(obstime[i]),linewidth=2)      
                    ax.plot(obstemp[j],obsDepth[j],'ro-',markersize=4, label=str(obstime[j]), linewidth=2)
...
for i in early_2013:
    for j in after_2013:
        if early_id_2013[i]==after_id_2013[j]:
            ...
            fig=plt.figure()
            ax=fig.add_subplot(111)
            plt.text(7,4,'id:'+str(early_id_2013[i])+'',fontsize=12)
            ax.set_xlim([5, 25])
            ax.set_ylim([50, 0])
            ax.set_xlabel(u'Tempature(°C)', fontsize=14)
            ax.set_ylabel('Depth(m)', fontsize=14)
