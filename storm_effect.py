        ...
        if early_id_2011[i]==after_id_2011[j]:
            if max(obsDepth[i])>30:
                if max(obsDepth[j])>30:
                    ...
                    plt.text(16,45,'before storm : '+str(obstime[i]),fontsize=10)
                    plt.text(16,48,'after storm : '+str(obstime[j]),fontsize=10)
                    ax.set_xlim([5, 30])
                    ax.set_ylim([50, 0])
                    ax.set_xlabel(u'Tempature(°C)', fontsize=14)
                    ax.set_ylabel('Depth(m)', fontsize=14)
                    ax.set_title('before and after storm',fontsize=16)
                    plt.xticks(fontsize=12)
                    plt.yticks(fontsize=12)
                    ax.plot(obstemp[i],obsDepth[i],'bo-',markersize=4, label='obs before',linewidth=1)      
                    ax.plot(obstemp[j],obsDepth[j],'bo--',markersize=4, label='obs after', linewidth=1)
                    #print obstemp[j]
                    ax.plot(modtemp[i],obsDepth[i],'ro-',markersize=4, label='mod before',linewidth=1)      
                    ax.plot(modtemp[j],obsDepth[j],'ro--',markersize=4, label='mod after', linewidth=1)
                    #print modtemp[j]
...
for i in early_2013:
    for j in after_2013:
        if early_id_2013[i]==after_id_2013[j]:
            ...
            ax.plot(modtemp[i],obsDepth[i],'ro-',markersize=4, label='mod before',linewidth=1)      
            ax.plot(modtemp[j],obsDepth[j],'ro--',markersize=4, label='mod after', linewidth=1)
            ax.legend(loc='upper left',fontsize='small')#, borderpad=1.5, labelspacing=1.5
