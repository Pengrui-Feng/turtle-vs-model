...
for i in range(2):
    ...
    ax1.plot(T[i][1], M[i], color='r', linewidth=1,label='modmean: '+str(T[i][3]))#when we want to plot the smoothed 'modILD' ,"T[i][0]['modILD']" changed with "M[i]' 
    ...
    #plt.text(T[i][0]['obsTime'][1],7,'obsmean: '+str(T[i][2]))
    #plt.text(T[i][0]['obsTime'][1],9,'modmean: '+str(T[i][3]))
...
#plt.savefig('obsVSmodVSweatherVSfvcom_im.png',dpi=200)
plt.show()
