...
draw the correlation of observation and models(fvcom,roms,hycom) between deep and shallow(50m)'
@author: yifan
"""
...
def show2pic(rng,tempobs, tempmod,fontsize,text,bins):
    ...
    for j in range(len(text)):
        ...
        if j==1:
           plt.text(x=1,y=32,s=r'$\mathregular{R^2}$='+str(round(r_squared,3)),fontsize=15)
        else:
           plt.text(x=1,y=27,s=r'$\mathregular{R^2}$='+str(round(r_squared,3)),fontsize=15)
    ...
    fig.text(0.94, 0.5, 'Quantity', ha='center', va='center', rotation='vertical',fontsize=FONTSIZE)
    plt.savefig('correlation comparison.png', dpi=200)
#########################################MAIN CODE#####################################################################################################
FONTSIZE = 25
##################################ship#########################################
...
for i in obsData.index:
    for j in range(len(obsdepth[i])):
        for k in range(len(shipdepth[i])):
            if obsdepth[i][j]==shipdepth[i][k]:
                tempObs_ship.append(obstemp[i][j])
